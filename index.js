...
var ExpressBrute = (module.exports = function(store, options) {
  ...
  while (this.delays[this.delays.length - 1] < this.options.maxWait) {
    var nextNum =
      ...
      (this.delays.length > 1 ? this.delays[this.delays.length - 2] : 0);
  ...
  if (typeof this.options.lifetime == 'undefined') {
    this.options.lifetime =
      ...
      1000 *
      (this.delays.length + this.options.freeRetries);
        ...
        this.store.get(
          ...
          _.bind(function(err, value) {
            ...
            if (
              ...
              remainingLifetime =
                remainingLifetime -
                Math.floor((this.now() - firstRequestTime) / 1000);
            ...
            if (
              nextValidRequestTime <= this.now() ||
              count <= this.options.freeRetries
            ) {
              this.store.set(
                key,
                {
